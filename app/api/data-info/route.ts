...
export async function GET(request: Request) {
    try {
        ...
        console.log(`Fetching paginated data: page ${page}, limit ${limit}`);

        const [equipements, greenSpaces, fountains] = await Promise.all([
            fetchAllData(APIs.equipements, page, limit),
            fetchAllData(APIs.greenSpaces, page, limit),
            fetchAllData(APIs.fountains, page, limit),
        ]);

        return NextResponse.json({ equipements, greenSpaces, fountains, page, limit });
    } catch (error) {
        console.error("API Fetch Error:", (error as Error).message);
        return NextResponse.json({ error: `Failed to fetch data: ${(error as Error).message}` }, { status: 500 });
    }
}
